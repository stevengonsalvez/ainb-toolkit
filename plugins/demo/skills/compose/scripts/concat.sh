...
DUR=$(/usr/bin/ffprobe -v error -show_entries format=duration -of csv=p=0 "$OUT")
/usr/bin/ffmpeg -loglevel error -y -i "$OUT" -vf "fps=12/${DUR%.*},scale=320:-1,tile=4x3" -frames:v 1 "$R/out/$NAME-montage.png"
