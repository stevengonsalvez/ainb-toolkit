...
METRICS_FILE="$HOME/.claude/metrics/costs.jsonl"
...
METRICS="$HOME/.claude/metrics/costs.jsonl"
if [[ -f "$METRICS" ]]; then
...
du -sh "$HOME/.claude/global-learnings/" 2>/dev/null || echo "  No knowledge base"
du -sh "$HOME/.claude/metrics/" 2>/dev/null || echo "  No metrics"
```
