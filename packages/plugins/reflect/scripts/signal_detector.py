...
HIGH_PATTERNS = [
    ...
    # Knowledge: root cause / fix confirmed
    (r"\b(root cause was|the issue was|the problem was|the bug was)\b", "root_cause"),
    (r"\b(fixed by|the fix was|the solution was|resolved by|solved by)\b", "fix_confirmed"),
    (r"\b(chose .+ over|decided to|went with)\b", "explicit_decision"),
    (r"\b(turns out|figured out|found it)\b", "breakthrough"),
]
...
MEDIUM_PATTERNS = [
    ...
    # Knowledge: effort-based
    (r"(spent|took)\s+\d+\s*(hour|min|day|h|m)", "time_investment"),
    (r"\b(the docs say .+ but|undocumented|misleading error)\b", "docs_vs_reality"),
    (r"\b(make sure to|requires .+ installed|only works)\b", "env_gotcha"),
]
...
LOW_PATTERNS = [
    ...
    (r"\b(seems to work|this approach works|so far so good)\b", "implicit_success"),
    (r"\b(discovered|switched (to|from)|replaced .+ with)\b", "tool_discovery"),
]
...
def run_tests():
    ...
    test_cases = [
        ...
        # High confidence - knowledge signals
        ("The root cause was a missing index", Confidence.HIGH, Category.UNKNOWN),
        ("Fixed by adding CASCADE to the migration", Confidence.HIGH, Category.UNKNOWN),
        ("Turns out the error was misleading", Confidence.HIGH, Category.UNKNOWN),
        ...
        # Medium confidence - knowledge signals
        ("Spent 3 hours tracking down the bug", Confidence.MEDIUM, Category.NEW_SKILL),
